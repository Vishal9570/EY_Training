# -*- coding: utf-8 -*-
"""colab3_github_actions_deploy.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1gvtS7iBBr6YhrUsZGEMpzDLklGP5oFdq
...
try:
    ...
    GITHUB_TOKEN   = userdata.get('')   # Personal Access Token (repo scope)
    GITHUB_USERNAME = userdata.get('UkkisulaLikhitha') # e.g. 'johndoe'
except:
    GROQ_API_KEY    = ''
    GITHUB_TOKEN    = ''
    GITHUB_USERNAME = 'UkkisulaLikhitha'
...
for p in [
    Path("/content/finsight-llm-eval"),
    Path("/content/finsight_eval_repo")
]:
    print("\nChecking:", p)
    if p.exists():
...
REPO_DIR = Path("/content/finsight-llm-eval")

print(REPO_DIR)
print(list((REPO_DIR / "src").glob("*")))
...
print("src and tests folders created")

print((REPO_DIR / "src").exists())
print((REPO_DIR / "tests").exists())

from pathlib import Path

print((REPO_DIR / "tests" / "test_eval_gates.py").exists())
...
print((REPO_DIR / "tests").exists())
print(list((REPO_DIR / "tests").glob("*")) if (REPO_DIR / "tests").exists() else "No tests folder")
...
git('git add .')
subprocess.run(
    ['git', 'commit', '-m', 'feat: initial LLM eval pipeline with GitHub Actions'],
    cwd=str(REPO_DIR)
)
...
print("SRC:", os.listdir(REPO_DIR / "src"))
print("TESTS:", os.listdir(REPO_DIR / "tests"))
...
!pip install bert-score transformers -q

with open("/content/finsight-llm-eval/src/test_cases.py", "r") as f:
    print(f.read())

with open("/content/finsight-llm-eval/src/run_ci_eval.py", "r") as f:
    print(f.read())

!python /content/finsight-llm-eval/src/run_ci_eval.py

from src.eval_harness import run_eval, build_leaderboard, check_quality_gate, GROQ_MODELS
from src.test_cases import SMOKE_TEST_CASES
...
leaderboard = build_leaderboard(results_all)
...
with open("results/gate.json", "w") as f:
    json.dump(gate, f, indent=2)
...
def make_key(model_name, test_case):
    raw = model_name + test_case["id"] + test_case["data"]
    return hashlib.md5(raw.encode()).hexdigest()

from src.eval_harness import run_eval, GROQ_MODELS
from src.test_cases import SMOKE_TEST_CASES
...
for model_name, model_id in GROQ_MODELS.items():
    ...
    for tc in SMOKE_TEST_CASES:
        ...
        key = make_key(model_name, tc)

        # 🔥 CACHE HIT
        if key in cache:
            print("✔ cache hit")
            results_all.append(cache[key])
            continue

        # ❌ CACHE MISS → run model
        result = run_eval([tc], models={model_name: model_id}, judge=True)[0]
...
from src.eval_harness import build_leaderboard, check_quality_gate
